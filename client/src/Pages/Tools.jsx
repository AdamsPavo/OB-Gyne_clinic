...
export default function Tools({ settingsOnly = false }) {
  const [tab, setTab] = useState(settingsOnly ? "clinic" : "services");
  ...
  return <div className="flex min-h-screen bg-slate-50">
    ...
    <div className="min-w-0 flex-1">
      <header className="clinic-page-header m-4 overflow-hidden rounded-3xl bg-linear-to-r from-violet-700 via-fuchsia-600 to-pink-500 text-white shadow-xl shadow-fuchsia-200/50 sm:m-6">
...
function Actions({ editing, close }) { return <div className="flex justify-end gap-2"><button type="button" onClick={close} className="rounded-xl border px-4 py-2.5 font-semibold text-slate-600">Cancel</button><PermissionButton module="tools" action={editing ? "edit" : "create"} className="inline-flex items-center gap-2 rounded-xl bg-fuchsia-600 px-4 py-2.5 font-semibold text-white"><Save size={17} />{editing ? "Update" : "Save"}</PermissionButton></div>; }
function IconButton({ icon: Icon, danger, onClick, label }) { return <PermissionButton module="tools" action={danger ? "delete" : "edit"} type="button" title={label} onClick={onClick} className={`rounded-lg p-2 ${danger ? "bg-red-50 text-red-600" : "bg-slate-100 text-slate-600"}`}><Icon size={16} /></PermissionButton>; }
...
function Charges({ items, reload, show }) {
  ...
  return <Panel icon={ReceiptText} title="Charge Types" description={`${items.length} reusable patient charges`} action={<PermissionButton module="tools" action={"create"} onClick={()=>setOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-fuchsia-600 px-4 py-2.5 font-semibold text-white"><Plus size={18}/>Add charge</PermissionButton>}>
...
function Inventory({ items, reload, show }) {
  ...
  return <Panel icon={Pill} title="Medicine Inventory" description={`${items.length} medicines · ${items.filter(i => +i.quantity <= +i.reorder_level).length} low stock`} action={<PermissionButton module="tools" action={"create"} onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-fuchsia-600 px-4 py-2.5 font-semibold text-white"><PackagePlus size={18} />Add medicine</PermissionButton>}>
