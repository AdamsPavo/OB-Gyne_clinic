...
function Editor({ title, record, fields, onSaved, extra = {}, saveLabel = "Save changes" }) {
  ...
  return <form onSubmit={async (e) => {
    ...
    <PermissionButton module="prenatal" action={/Delivered|Completed|Closed/.test(String(extra.status || form.status || "")) ? "complete" : "edit"} disabled={saving} className={`${button} mt-4`}>{saving ? "Saving..." : saveLabel}</PermissionButton>
    ...
    <section className={card}><h2 className="text-lg font-bold">Complete prenatal consultation timeline</h2>
      <div className="mt-4 space-y-3">{record.visits.map((visit, index) => <details key={visit.id} open={index === record.visits.length - 1} className="rounded-2xl border border-slate-200 p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-4">{visit.consultation_case_id && <Link className="text-sm font-semibold text-pink-700 underline" to={`/cases/${visit.consultation_case_id}`}>Consultation details / print</Link>}
          {siblings.length > 1 && <PermissionButton module="prenatal" action={"edit"} className="text-sm text-slate-500 underline" onClick={() => { setMoving(visit.id); setMoveTarget(""); setMoveMessage(""); }}>Move to another pregnancy</PermissionButton>}
        </div>
      </details>)}</div>
      ...
      {moving && <form className="mt-4 rounded-xl bg-amber-50 p-4" onSubmit={async (e) => { e.preventDefault(); if (moveSaving) return; setMoveSaving(true); try { await api(`/pregnancies/${moveTarget}/visits/${moving}`, { method: "PUT", body: "{}" }); setMoving(null); await onSaved(); } catch (error) { setMoveMessage(error.message); } finally { setMoveSaving(false); } }}>
        ...
        <PermissionButton module="prenatal" action={"edit"} disabled={moveSaving} className={`${button} mt-3`}>Move visit</PermissionButton><button type="button" className="ml-3 text-sm" onClick={() => setMoving(null)}>Cancel</button>{moveMessage && <p role="alert">{moveMessage}</p>}
      </form>}
    </section>
...
export default function PregnancyRecords() {
  ...
  return <div className="flex min-h-screen bg-slate-50"><Sidebar activeItem="Prenatal Records" /><div className="min-w-0 flex-1">
    <header className="clinic-page-header m-4 flex flex-wrap items-center justify-between gap-4 rounded-3xl bg-linear-to-r from-pink-700 to-rose-500 p-6 text-white sm:m-6"><div><p className="text-sm text-pink-100">Maternal health</p><h1 className="text-3xl font-bold">Prenatal Records</h1><p className="mt-1 text-sm text-pink-100">One pregnancy record, from first visit through delivery.</p></div><div className="flex flex-wrap gap-2"><PermissionButton module="prenatal" action="print" onClick={() => authorizedAction("prenatal","print",() => window.print())} className="rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-pink-700">Print record</PermissionButton><PermissionButton module="prenatal" action={"create"} onClick={() => setNewForm(!newForm)} className="rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-pink-700">Start new pregnancy</PermissionButton><Link className="rounded-xl border border-white/50 px-4 py-2.5 text-sm font-semibold" to={`/consultations/new?prenatal=1${patientFilter ? `&patient=${patientFilter}` : ""}`}>Add prenatal consultation</Link></div></header>
